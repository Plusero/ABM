...
from experiments.steering_2d import run_simulation
from experiments.visualization import steering_visuals


@click.command()
@click.option('--n_seeds', type=int, default=1)
@click.option('--init_seed', type=int, default=0)
@click.option('--scenario', type=str, default="enclosure")
@click.option('--n_agents', type=int, default=16)
@click.option('--n_school', type=int, default=10)
@click.option('--n_predators', type=int, default=0)
...
@click.option('--kox_s', type=float, default=2500.)
@click.option('--koz_s', type=float, default=500.)
...
@click.option('--sigma_s', type=float, default=0.1)
@click.option('--v_a_max', type=float, default=4.)
@click.option('--v_s_max', type=float, default=2.)
@click.option('--v_z_max', type=float, default=1.8)
...
@click.option('--zoi_d', type=float, default=40.0)
@click.option('--mu', type=float, default=19.)
@click.option('--eta', type=float, default=0.5)
@click.option('--tau', type=float, default=0.2)
@click.option('--t_z', type=int, default=900)
@click.option('--t_z_end', type=int, default=1150)
@click.option('--t_max', type=int, default=1150)
...
def run(n_seeds=1,
    ...
    filename = 'POSES_STEERING_TYPE={TYPE}_' + \
    ...
    for seed in range(init_seed, init_seed + n_seeds):
        school_poses, agent_poses, patch_poses, enclosure_pose = run_simulation(scenario=scenario,
                                                                                n_agents=n_agents,
                                                                                n_school=n_school,
        ...
        if scenario == "patches":
            zone_poses = patch_poses
        elif scenario == "enclosure":
            zone_poses = enclosure_pose
        else:
            zone_poses = np.zeros(1)
        ...
        np.save(save_folder + filename.format(TYPE="Z", SEED=seed) + ".npy", zone_poses)

        if visualization:
            steering_visuals.run(seed, scenario, school_poses, agent_poses, zone_poses)
